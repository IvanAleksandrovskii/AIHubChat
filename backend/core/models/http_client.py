...
# Named it like this just for fun—now it's immediately clear that it's a client manager.
# "ClientManager" is way too generic 🤣
# The code below is there to prevent the server from being overloaded with too many open clients.
class UberClient:
...
class ClientManager:
    def __init__(
        self,
        client_timeout=settings.http_client.timeout,
    ...
    async def cleanup_inactive_clients(self) -> None:
        ...
        async with self.lock:
            ...
            self.clients = [
                ...
                if client.is_busy
                or (current_time - client.last_used < self.client_timeout)
            ]
    ...
    async def get_client(self) -> UberClient:
        ...
        async with self.lock:
            ...
            available_clients = [
                client for client in self.clients if not client.is_busy
            ]
            log.info(
                f"Cleanup completed. %s clients remaining.", len(available_clients)
            )
            ...
            if len(self.clients) < self.max_clients:
                new_client = UberClient(
                    httpx.AsyncClient(timeout=self.client_timeout, limits=self.limits)
                )
